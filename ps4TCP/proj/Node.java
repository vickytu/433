...
public class Node {
    ...
    public void start() {
	logOutput("started");
	this.addTimer(PingTimeout, "pingTimedOut", null, null, this);
     ...
     */
    /* doesn't work since I modified it, but IDK why */
    ...
    /* added for TCP by Vicky */
    case Protocol.TRANSPORT_PKT:
        tcpMan.receiveTCP(packet);
        break;
    ...
    /* modified to be used for TCP pkt timeout */
    // Adds a timer, to fire in deltaT milliseconds, with a callback to a public function of this class that takes no parameters
    public void addTimer(long deltaT, String methodName, String[] paramTypes, Object[] obj, Object parent) {
	try {
	    Method method = Callback.getMethod(methodName, parent, paramTypes);
	    Callback cb = new Callback(method, parent, obj);
    ...
    private boolean matchTransferCommand(String command) {
        ...
        try {
            ...
            logError("in matchTransferCommand");
            logError("Exception: " + e);
        }
    ...
    private boolean matchServerCommand(String command) {
        ...
        try {
            ...
            logError("in matchServerCommand");
            logError("Exception: " + e);
        }
